%{
    Estimate bid distributions for incumbent and entrant from net auctions sample.
    This file gives us the compound net cost signal RHO.
    
    KEY OUTPUT FROM THIS FILE: Vector of RHO for each bidder type and
    auction. 
    
    IMPORTANT: Use exactly the same X_orig-matrix specification as for gross auction
    estimation.
...
global data N_obs K data_sim data_sim_symmetric
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% IMPORTANT TUNING OPERATIONS IN THIS STEP.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Set threshold at which we truncate the net cost signal RHO.
% Negative signals are possible if revenues are larger than costs.
% This coudl be a function of track characteristics etc. However, with the cleaned sample it doesn't matter much.
neg_RHO_threshold = -100; 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% END OF IMPORTANT TUNING OPERATIONS IN THIS STEP.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% Read-in data.
% Read header line for variable names.
fid = fopen(project_paths('OUT_DATA','na_matlab.csv'),'r');
headerline  = fgetl(fid);
data_legend = textscan(headerline,'%s','Delimiter',',');
data_legend = data_legend{1,1}';
% Read data.
data = dlmread(project_paths('OUT_DATA','na_matlab.csv'),',',1,0);
...
% Trim number of bidders and winning bids.
N_max = 5 .* cons;
N = min(N,N_max);
bid_min = 0 .* cons;
bid_win = max(bid_win,bid_min);
% Construct trend variable for exploratory purposes.
...
% entrant.) IMPORTANT: Use same regressors as for gross auctions!
...
% Can also try with fminsearch to get initial idea on potentail parameter values.
...
[std_errors, t_stats, p_values] = stats_net(theta_opt, min_neg_log_ll, 10^-8, 2);


% Write estimation results for net bid functions to file.
disp('Estimation results for net auctions:')
results_net = [theta_opt, std_errors, t_stats, p_values];
col_labels = {'Point Estimates', 'Standard Errors', 't-Statistics', 'p-Values'};
row_labels = {'$\lambda^I_X$', '','','','', '$\lambda^I_N$','$\lambda^E_X$', '', '', '','','$\lambda^E_N$','$\nu^I_X$', '','','','', '$\nu^I_N$','$\nu^E_X$','','','','','$\nu^E_N$'};
matrix2latex(results_net,project_paths('OUT_TABLES','resultsnet.tex'),'headerRow', col_labels, 'headerColumn', row_labels, 'format', '%6.4f', 'caption', 'Estimation results: bid functions in net auctions', 'label', 'est_results_net');
% Save estimation results in raw format to be formatted in a nicer table using Python.
dlmwrite(project_paths('OUT_ANALYSIS','erbf_net.csv'),results_net);
fid = fopen(project_paths('OUT_ANALYSIS','erbf_net_legend.csv'), 'w');
fprintf(fid, '%s,', row_labels{1:19});
fclose(fid);


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Predict winning bid parameters based on theta_opt.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Compute bid distribution parameters for each line for both incumbent and
% entrant.
% Combine regressors for incumbent and entrant.
...
% Define range of bid function and (net) cost plots.
...
if update_plots==1
    ...
    bid_grid = repmat(linspace(0,100,n_bid_grid),N_obs,1);
    ...
    for i=1:N_obs
        ...
        subplot(1,1,1)
        CDF_bid_est = plot(bid_grid(i,:), bid_distribution_I(i,:), bid_grid(i,:), bid_distribution_E(i,:));
        graphtitle = sprintf(strcat('Comparison of bid CDF for net contract ',num2str(i)));
        title(graphtitle, 'Interpreter','latex');
        legend('Incumbent', 'Entrant', 'Interpreter','latex')
        axis([x_axis_bid(i,1) x_axis_bid(i,2) 0 1])
        filename = sprintf(regexprep(strcat(project_paths('OUT_FIGURES','net_bidcdf'),num2str(i)),'\','\\\'));
        saveas(gcf,filename,'pdf');
    end
end

% Save postestimation workspace for net auctions. This file is only for diagnostics.
save(project_paths('OUT_ANALYSIS','net_step1'));

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Back out winner's net cost signal for each observed net auction.
%  The net cost signal is RHO in our model.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
RHO = zeros(N_obs,2);

% Compute CDF and PDF of incumbent for each observed winning bid.
[CDF_I, PDF_I] = eval_bf(lambda_I,rho_I,y,truncation_bid);
% Compute CDF and PDF of entrant for each observed winning bid.
[CDF_E, PDF_E] = eval_bf(lambda_E,rho_E,y,truncation_bid);


%% Compute probability of DB being pivotal (used for computing conditional probability in conditional expectation revenue term).
% Part 1: Numerator
db_pr_num = PDF_I .* (1-CDF_E).^(N-2);
% Part 2: Denominator (consisting of part for DB and the remaining N-2
% rival entrants).
db_pr_den = PDF_I .* (1-CDF_E).^(N-2) + (N-2) .* PDF_E .* (1-CDF_I) .* (1-CDF_E).^(N-3);
% Combine terms.
db_prob = db_pr_num ./ db_pr_den;
%db_prob_backup = db_prob;
% Safety check, so that prob of DB being pivotal is one for N=2 auctions for sure.
db_prob(N==2) = 1;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Compute markup terms for incumbent.
% Numerator of markup term.
G_MI = (1-CDF_E).^(N-1);
% Denominator of markup term.
g_MI = (N-1) .* (1-CDF_E).^(N-2) .* PDF_E;
% Full markup term.
MU_I = G_MI ./ g_MI;
% Compute vector of incumbent's net costs signal.
% This vector is only non-zero if DB wins.
netcost_I_win_na = db_win .* (y - MU_I);
% First column contains Rho for incumbent.
RHO(:,1) = (y - MU_I);

% Check how many net cost draws are negative.
sprintf('Number of negative net cost signals for incumbent:')
sum(netcost_I_win_na<0)
...
% Full markup term.
MU_E = G_ME ./ g_ME;
% Compute vector of entrants' net costs.
% This vector is only non-zero if entrant wins.
netcost_E_win_na =(1-db_win) .* (y - MU_E);
% Second column contains Rho for entrant.
RHO(:,2) = (y - MU_E);
% Check how many net cost draws are negative.
sprintf('Number of negative winning net costs for entrant:')
sum(netcost_E_win_na<0)
% Combine vector of winning net costs.
netcost_win_na = netcost_I_win_na + netcost_E_win_na;
% Truncate net cost signals at plausible negative value.
% Mostly for diagnostics. Shouldn't matter too much for cleaned sample.
RHO(RHO < neg_RHO_threshold) = neg_RHO_threshold;
max(RHO)
mean(RHO)
min(RHO)
...
subplot(2,1,1)
netcost_win_I_plot = netcost_I_win_na(db_win==1);
cdfplot(netcost_win_I_plot);
title('Net cost distribution of winning incumbent','Interpreter','latex')
axis([-30 30 0 1])

subplot(2,1,2)
netcost_win_E_plot = netcost_E_win_na(db_win==0);
cdfplot(netcost_win_E_plot);
title('Net cost distribution of winning entrant','Interpreter','latex')
axis([-30 30 0 1])
filename = project_paths('OUT_FIGURES','netcostwinnet');
saveas(gcf,filename,'pdf');


%% Compute bid functions as function of RHO (including expectation about rivals' revenue signals.
% Compute bid grid.
n_bid_grid = 5000;
lambda_I_grid = repmat(lambda_I,1,n_bid_grid);
lambda_E_grid = repmat(lambda_E,1,n_bid_grid);
...
for t=1:N_obs
    lambda_I_sim = lambda_I(t,1);
    lambda_E_sim = lambda_E(t,1);
    rho_I_sim = rho_I(t,1);
    rho_E_sim = rho_E(t,1);
        ...
        % Compute CDF and PDF for markup computation and for both types at winning bid.
        ...
        title_str = sprintf(strcat('Compound net bid function (incumbent) for net contract  ',num2str(t)));
        title(title_str,'Interpreter','latex')
        axis([-15 bid_grid(t,n_bid_grid./10) 0 bid_grid(t,800)])
        %yyaxis right;
        %ylabel('Cost density')
        xlabel('Net Cost Signal $\mathcal{P}$ (in 10 Mio EUR)', 'Interpreter','latex')
        ...
        ylabel('Bid (in 10 Mio EUR)', 'Interpreter','latex')
        title_str = sprintf(strcat('Compound net bid function (entrant) for net contract  ',num2str(t)));
        title(title_str, 'Interpreter','latex')
        axis([-15 bid_grid(t,n_bid_grid./10) 0 bid_grid(t,800)])
        %yyaxis right;
        %ylabel('Cost density')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% END OF IMPORTANT TUNING PARAMETERS              %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for t=1:size(X_orig,1);
    ...
    % Compute markup CDF and PDF for both types at winning bid.
    ...
    MU_E_plot = MU_E(MU_E<prctile(MU_E,97.5) & MU_E>prctile(MU_E,2.5));
    MU_I_plot = MU_I(MU_I<prctile(MU_I,97.5) & MU_I>prctile(MU_I,2.5));
    fprintf('Median markup for entrant and incumbent in auction %d: %6.4f and %6.4f\n',t, nanmedian(MU_I), nanmedian(MU_E));
    fprintf('Median markup for entrant and incumbent in auction %d (outliers cleaned): %6.4f and %6.4f\n',t, nanmedian(MU_I_plot), nanmedian(MU_E_plot));
    fprintf('Average markup for entrant and incumbent in auction %d: %6.4f and %6.4f\n',t, nanmean(MU_I), nanmean(MU_E));
    fprintf('Average markup for entrant and incumbent in auction %d (outliers cleaned): %6.4f and %6.4f\n',t, nanmean(MU_I_plot), nanmean(MU_E_plot));
    ...
    [mu_density_I,mu_grid_I] = ksdensity(MU_I_plot);
        ...
        fprintf('Weird markup distribution for incumbent in net auction %d. Investigate!\n',t);
    ...
    try
        ...
        fprintf('Weird markup distribution for entrant in net auction %d. Investigate!\n',t);
    ...
    graphtitle = sprintf('Comparison of markup distribution for entrant and incumbent for net contract %d' ,t);
    title(graphtitle);
    legend('Incumbent', 'Entrant')
    filename = sprintf(regexprep(strcat(project_paths('OUT_FIGURES','mu_comp_net'),num2str(t)),'\','\\\'));
    saveas(gcf,filename,'pdf');
    ...
    if max(G_MI)>0 && max(G_ME)>0
        ...
        subplot(2,1,1)
        cdfplot(cost_I_grid);
        title_str = sprintf(strcat('Implied IPV-cost CDF of incumbent for net contract ',num2str(t)));
        title(title_str, 'Interpreter','latex')
        axis([x_axis_cost(t,1) x_axis_cost(t,2) 0 1])
        % Plot empirical CDF of entrant's cost in second subplot.
        subplot(2,1,2)
        cdfplot(cost_E_grid);
        title_str = sprintf(strcat('Implied IPV-cost CDF of entrant for net contract ',num2str(t)));
        title(title_str, 'Interpreter','latex')
        axis([x_axis_cost(t,1) x_axis_cost(t,2) 0 1])
        filename = sprintf(regexprep(strcat(project_paths('OUT_FIGURES','costcdfnetipv'),num2str(t)),'\','\\\'));
        saveas(gcf,filename,'pdf');

        %% Kernel densities of costs.
        % Plot kernel density of incumbent's cost in first subplot.
        subplot(2,1,1)
        [kdens_I, kdens_I_grid] = ksdensity(cost_I_grid, bid_grid(t,:),'bandwidth',kd_bandwidth,'NumPoints', N_gridpoints_kd);
        % Compute expected cost for incumbent by numerical integration over kernel density.
        cost_quantiles(t,1,1) = trapz(kdens_I_grid,kdens_I_grid .* kdens_I);
        ...
        kdens_I = kdens_I ./ (1-neg_cost_I);
        % Set negativ density part to zero.
        kdens_I(kdens_I_grid<0) = 0;
        % Plot kernel density.
        plot(kdens_I_grid,kdens_I)
        title_str = sprintf(strcat('Kernel density of incumbent IPV-costs for net contract ',num2str(t)));
        title(title_str, 'Interpreter','latex')
        axis([x_axis_cost(t,1) x_axis_cost(t,2) 0 0.75])
        % Do the same thing for entrant cost distribution.
        subplot(2,1,2)
        [kdens_E, kdens_E_grid] = ksdensity(cost_E_grid,bid_grid(t,:), 'bandwidth',kd_bandwidth,'NumPoints', N_gridpoints_kd);
        % Compute expected cost for entrant by numerical integration.
        cost_quantiles(t,1,2) = trapz(kdens_E_grid,kdens_E_grid .* kdens_E);
        ...
        kdens_E = kdens_E ./ (1-neg_cost_E);
        kdens_E(kdens_E_grid<0) = 0;
        plot(kdens_E_grid,kdens_E);
        title_str = sprintf(strcat('Kernel density of entrant IPV-costs for net contract ',num2str(t)));
        title(title_str, 'Interpreter','latex')
        axis([x_axis_cost(t,1) x_axis_cost(t,2) 0 0.6])
        filename = sprintf(regexprep(strcat(project_paths('OUT_FIGURES','kdensitycostnetipv'),num2str(t)),'\','\\\'));
        saveas(gcf,filename,'pdf');
        ...
        yyaxis left;
        plot(signal_I_bf(t,:),bid_grid(t,:));
        ylabel('Bid in UNIT???', 'Interpreter','latex')
        title_str = sprintf(strcat('Estimated IPV bid function and cost distribution (incumbent) for net contract  ',num2str(t)));
        title(title_str, 'Interpreter','latex')
        axis([bid_grid(t,1) bid_grid(t,n_bid_grid./10) 0 bid_grid(t,n_bid_grid./10)])
        yyaxis right;
        plot(bid_grid(t,:), kdens_I);
        ylabel('Cost density', 'Interpreter','latex')
        xlabel('Cost Signal in UNIT???', 'Interpreter','latex')
        ...
        plot(signal_E_bf(t,:),bid_grid(t,:));
        ylabel('Bid in UNIT???', 'Interpreter','latex')
        title_str = sprintf(strcat('Estimated IPV bid function and cost distribution (entrant) for net contract  ',num2str(t)));
        title(title_str, 'Interpreter','latex')
        axis([bid_grid(t,1) bid_grid(t,n_bid_grid./10) 0 bid_grid(t,n_bid_grid./10)])
        yyaxis right;
        plot(bid_grid(t,:), kdens_E);
        ylabel('Cost density', 'Interpreter','latex')
        xlabel('Cost Signal in UNIT???', 'Interpreter','latex')
        filename = sprintf(regexprep(strcat(project_paths('OUT_FIGURES','bfkd_net_ipv'),num2str(t)),'\','\\\'));
        saveas(gcf,filename,'pdf');
        ...
        % Test for FOSD on full range: 0 to 90%-percentile of bid function.
        c_full_max = max(cost_quantiles(t,8,1),cost_quantiles(t,8,2));
        c_lower_max = max(cost_quantiles(t,4,1),cost_quantiles(t,4,2));
        a_grid_full = linspace(0,c_full_max,N_a);
        % Test for FOSD only on lower tail: 0 to 6. (we may need to update what makes sense here or make this line-specific...)
        a_grid_lower = linspace(0,c_lower_max,N_a);
        ...
        eCDF_cost_full(t,:,2) = ksdensity(cost_E_grid, a_grid_full, 'bandwidth',kd_bandwidth,'NumPoints', N_gridpoints_kd, 'function','cdf');
        % Compute variance of empirical CDF for FOSD test.
        var_a_fosd_full(t,:,1) = (eCDF_cost_full(t,:,1) - eCDF_cost_full(t,:,1).^2) ./ N_a;
        var_a_fosd_full(t,:,2) = (eCDF_cost_full(t,:,2) - eCDF_cost_full(t,:,2).^2) ./ N_a;
        % Covariance of F_I and F_E.
        cov_FI_FE_full(t,:) = (1 - eCDF_cost_full(t,:,1) .* eCDF_cost_full(t,:,2)) ./ N_a;
        % Compute variance.
        var_a_fosd_full(t,:,3) = var_a_fosd_full(t,:,1) + var_a_fosd_full(t,:,2);
        % Alternative variance formula from Davidson/Duclos.
        var_a_fosd_full(t,:,4) = - (eCDF_cost_full(t,:,1) - eCDF_cost_full(t,:,2)) .*  (eCDF_cost_full(t,:,2) - eCDF_cost_full(t,:,1)) ./ N_a;
        % Compute test statistic.
        T_grid_full(t,:) = (eCDF_cost_full(t,:,2) - eCDF_cost_full(t,:,1)) ./ sqrt(var_a_fosd_full(t,:,3));
        % Compute rejection decision.
        reject_equality(t,1) = max(-T_grid_full(t,:) > m_kalpha) .* min(T_grid_full(t,:) < m_kalpha);
        ...
        eCDF_cost_lower(t,:,2) = ksdensity(cost_E_grid, a_grid_lower, 'bandwidth',kd_bandwidth,'NumPoints', N_gridpoints_kd, 'function','cdf');
        % Compute variance of empirical CDF for FOSD test.
        var_a_fosd_lower(t,:,1) = (eCDF_cost_lower(t,:,1) - eCDF_cost_lower(t,:,1).^2) ./ N_a;
        var_a_fosd_lower(t,:,2) = (eCDF_cost_lower(t,:,2) - eCDF_cost_lower(t,:,2).^2) ./ N_a;
        % Covariance of F_I and F_E.
        cov_FI_FE_lower(t,:) = (1 - eCDF_cost_lower(t,:,1) .* eCDF_cost_lower(t,:,2)) ./ N_a;
        % Compute variance.
        var_a_fosd_lower(t,:,3) = var_a_fosd_lower(t,:,1) + var_a_fosd_lower(t,:,2);
        % Alternative variance formula from Davidson/Duclos.
        var_a_fosd_lower(t,:,4) = - (eCDF_cost_lower(t,:,1) - eCDF_cost_lower(t,:,2)) .*  (eCDF_cost_lower(t,:,2) - eCDF_cost_lower(t,:,1)) ./ N_a;
        % Compute test statistic.
        T_grid_lower(t,:) = (eCDF_cost_lower(t,:,2) - eCDF_cost_lower(t,:,1)) ./ sqrt(var_a_fosd_lower(t,:,3));
        % Compute rejection decision.
        reject_equality(t,2) = max(-T_grid_lower(t,:) > m_kalpha) .* min(T_grid_lower(t,:) < m_kalpha);
...
fprintf(fid,':SUMMARY STATISTICS WHEN TESTING FOR FOSD OF COST DISTRIBUTION UNDER IPV SETTING ASSUMPTION:\n')
fprintf(fid,'Reject equality of entire distribution in favor of incumbent having better cost distribution on %6.4f net lines.\n ', sum_reject_Fcost_equality(1));
fprintf(fid,'Reject equality of lower part of distribution in favor of incumbent having better cost distribution on %6.4f net lines.\n ', sum_reject_Fcost_equality(2));
fprintf(fid,'\nEXPRESSED IN SAHRE OF TOTAL NUMBER OF LINES IN NET SAMPLE:\n')
...
saveas(gcf,project_paths('OUT_FIGURES','hist_mean_mu_net'),'pdf');
...
saveas(gcf,project_paths('OUT_FIGURES','hist_median_mu_net'),'pdf');

% Write markup statistic to table.
% Two files for absolute and relative markup statistics, respectively.
...
writetable(mu_table_abs, project_paths('OUT_ANALYSIS','mu_stats_net_abs.csv'));
mu_table_rel = table(MU_rel(:,1),MU_rel(:,2),MU_rel(:,3),MU_rel(:,4),'VariableNames', mu_stats_labels);
writetable(mu_table_rel, project_paths('OUT_ANALYSIS','mu_stats_net_rel.csv'));
...
%% Save essential workspace for second step ML.
clear truncation_bid theta_0 update_plots row_labels rho_vec rho_I_grid ...
    rho_I rho_E_grid rho_E PDF_I PDF_E parameter_matrix net neg_log_ll_opt ...
    n_bid_grid MU_I MU_E min_neg_log_ll mI_grid mE_grid margin_incumbent ...
    margin_entrants margin lambda_vec lambda_I_grid lambda_I lambda_E_grid lambda_E ...
    I_grid i gross graphtitle G_MI g_MI g_ME g_MI fminsearch_options filename ...
    fid eCDF_mI eCDF_mE eCDF_I eCDF_I eCDF_E E_grid data_sim_symmetric data_sim ...
    col_labels CDF_plot_I CDF_plot_E CDF_I CDF_E bid_min bid_distribution_I bis_distribution_E ...
    bid_density_I bid_density_E avg_bid_distribution_I avg_bid_distribution_E ans ...
    X_aux netcost_win_I_plot netcost_win_E_plot G_ME CDF_Bid_est bid_grid ...
    bid_distribution_E CDF_bid_est
save(project_paths('OUT_ANALYSIS','na_2step'));